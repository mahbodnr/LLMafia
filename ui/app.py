...
class GameManager:
    ...
    def save_uploaded_transcript(self, transcript_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            fd, self.transcript_path = tempfile.mkstemp(
                suffix=".json", prefix="mafia_transcript_"
            )
    ...
    def _create_role_distribution(self, settings: Dict[str, Any]) -> Dict[str, int]:
        ...
        roles = {
            ...
            - (1 if settings["includeGodfather"] else 0),
        }
    ...
    def _create_game_config(
        self, settings: Dict[str, Any], roles: Dict[str, int]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "ai_models": [
                ...
                # {"provider": "ollama", "model": "tulu3:70b", "base_url": os.environ.get("OLLAMA_URL", "http://localhost:11434")},
                # {"provider": "ollama", "model": "qwen2.5:72b", "base_url": os.environ.get("OLLAMA_URL", "http://localhost:11434")},
                {"provider": "ollama", "model": "llama3.3:70b", "team": "mafia", "base_url": os.environ.get("OLLAMA_URL", "http://localhost:11434")},
                {"provider": "ollama", "model": "deepseek-r1:70b", "team": "village", "base_url": os.environ.get("OLLAMA_URL", "http://localhost:11434")},
            ],
            "mechanics": {
    ...
    def execute_phase(self):
        ...
        logger.info(
            f"Executing phase {self.game.game_state.current_phase.name} in background"
        )
    ...
    def handle_message(self, message: Message):
        ...
        try:
            ...
            if (
                self.game
                and self.game.game_state
                and "discussion" in self.game.game_state.current_phase.name.lower()
            ):
    ...
    def display_next_action(self):
        ...
        if not self.night_actions_queue or self.current_action_index >= len(
            self.night_actions_queue
        ):
            return
...
def handle_next_phase():
    ...
    if (
        not game_manager.game
        or not game_manager.game.game_state
        or game_manager.game.game_state.game_over
    ):
        return
...
def handle_get_transcript(callback=None):
    ...
    try:
        ...
        transcript_file = game_manager.game.save_transcript()
        ...
        try:
            ...
            if callback:
                ...
                logger.info(
                    f"Transcript sent to client: {len(str(transcript_data))} bytes, {len(transcript_data.keys()) if isinstance(transcript_data, dict) else 'not a dict'} keys"
                )
        ...
        if hasattr(player, "memory") and player.memory:
            for entry in player.memory:
                ...
                if processed_entry["type"] == "inner_thought" or (
                    processed_entry["type"] == "message"
                    and processed_entry.get("sender") == player_name
                ):
                ...
                for vote in votes:
                    try:
                        ...
                        if voter_id and target_id:
                            voter_name = game_manager.game.game_state.players[
                                voter_id
                            ].name
                            target_name = game_manager.game.game_state.players[
                                target_id
                            ].name
                            ...
                            if (
                                hasattr(vote, "phase")
                                and getattr(vote, "phase") is not None
                            ):
                            ...
                            if voter_name == player_name or target_name == player_name:
                                ...
                                logger.info(
                                    f"Added vote in round {vote_round}, phase {vote_phase}: {voter_name} -> {target_name}"
                                )
                ...
                for action in actions:
                    try:
                        ...
                        if actor_id and target_id:
                            actor_name = game_manager.game.game_state.players[
                                actor_id
                            ].name
                            target_name = game_manager.game.game_state.players[
                                target_id
                            ...
                            if (
                                ...
                                and getattr(action, "phase") is not None
                            ):
                            ...
                            if actor_name == player_name:
                                ...
                                logger.info(
                                    f"Added action in round {action_round}, phase {action_phase}: {actor_name} {action_entry['action_type']} {target_name}"
                                )
...
def handle_transcript_upload(data):
    ...
    try:
        ...
        if not data.get("content"):
            ...
            emit(
                "transcript_upload_response",
                {"success": False, "error": "No content in uploaded transcript"},
            )
            return
        ...
        if game_manager.save_uploaded_transcript(data["content"]):
            emit(
                "transcript_upload_response",
                {"success": True, "filename": data.get("filename", "unknown")},
            )
        else:
            emit(
                "transcript_upload_response",
...
def emit_game_over():
    ...
    try:
        ...
        winning_team = (
            "Village"
            if game_manager.game.game_state.winning_team == TeamAlignment.VILLAGE
            else "Mafia"
        )
