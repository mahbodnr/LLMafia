...
class MafiaGame:
    ...
    def _save_transcript(self):
        ...
        transcript = {
            "config": self.config,
            "players": {pid: {"name": p.name, "role": p.role.name, "team": p.team.name} 
                       for pid, p in self.game_state.players.items()},
            "events": [{"round": e.round_num, "phase": e.phase.name, "type": e.event_type, 
                        "description": e.description, "public": e.public} 
                      for e in self.game_state.events],
            "messages": [{"round": m.round_num, "phase": m.phase.name, "sender": m.sender_id, 
                         "content": m.content, "public": m.public} 
                        for m in self.game_state.messages],
            "votes": [{"round": v.round_num, "phase": v.phase.name, "voter": v.voter_id, 
                      "target": v.target_id} 
                     for v in self.game_state.votes],
            "actions": [{"round": a.round_num, "phase": a.phase.name, "actor": a.actor_id, 
                        "action": a.action_type, "target": a.target_id, "success": a.success} 
                       for a in self.game_state.actions],
            "result": {
                "game_over": self.game_state.game_over,
                "winning_team": self.game_state.winning_team.name if self.game_state.winning_team else None
            }
        }
...
def main():
    ...
    parser.add_argument('--godfather', type=bool, default=True, help='Include godfather role')
    parser.add_argument('--doctor', type=bool, default=True, help='Include doctor role')
    parser.add_argument('--detective', type=bool, default=True, help='Include detective role')
    parser.add_argument('--rounds', type=int, default=3, help='Number of discussion rounds per day')
    ...
    roles = {
        "Villager": args.players - args.mafia - (1 if args.godfather else 0) - 
                   (1 if args.doctor else 0) - (1 if args.detective else 0),
        "Mafia": args.mafia,
    }
